...
(defn- http-header-edn-function?
  ...
  Builds `{:name \"…\" :value \"…\"}` when both atoms pass quote/backslash scan;
  otherwise empty. WAT owns the dual scan + concat; admission matches export
  name containing header_edn / header-edn (not trust) and a body that mentions
  both params plus concat / reject-empty / quoted skeleton."
  [{:keys [name params param-types result body]}]
  (and (= 2 (count params))
...
(defn- headers-edn-append-function?
  ...
  (and (= 3 (count params))
       ...
       (let [nm (clojure.core/name name)
             ...
             n (nth params 1)
             v (nth params 2)]
         (and (re-find #"(?i)headers?[_-]?edn[_-]?append" nm)
              (form-tree-walk body #(= % acc))
              (form-tree-walk body #(= % n))
              (form-tree-walk body #(= % v))
              (or (form-tree-walk body #(= % 'string-concat))
                  (form-tree-walk body #(= % 'http_header_edn))
                  (form-tree-walk body #(= % 'headers_edn_has_name))
                  (form-tree-walk body #(= % 'string-substring))
                  (form-tree-walk body #(= % "")))
              (or (form-tree-walk body #(= % "[]"))
                  (form-tree-walk body #(= % "["))
...
(defn- http-result-ok-edn-function?
  "Reject-path result ok arm (T8.3 composition on edn_quoted):
  i64×string×string → string.

  Builds `{:tag :ok :status N :headers […] :body \"…\"}` when status ∈ [0,999],
  headers are `[…]` shape, and body passes quote/backslash scan. WAT owns
  decimal status + scan + shape gate (no full multi-export request package)."
  [{:keys [name params param-types result body]}]
  (and (= 3 (count params))
       ...
       (let [nm (clojure.core/name name)
             ...
             body-p (nth params 2)]
         (and (re-find #"(?i)result[_-]?ok[_-]?edn" nm)
              ...
              (or (form-tree-walk body #(= % 'string-concat))
                  ...
                  (form-tree-walk body #(= % "")))
              (or (form-tree-walk body #(= % "ok"))
                  (form-tree-walk body #(and (string? %) (str/includes? % "status")))
                  (form-tree-walk body #(and (string? %) (str/includes? % "headers")))
                  (form-tree-walk body #(= % 'string-byte-length))
                  (form-tree-walk body #(= % 'string-length))
                  (form-tree-walk body #(= % "")))))))

(defn- http-request-edn0-function?
  "Reject-path 0-header request EDN (T8.3 / ADR 0209 composition on edn_quoted):
  string×string×i64 → string.

  Builds `{:url \"…\" :headers [] :body \"…\" :timeout-ms N}` when url and body
  pass quote/backslash scan and timeout-ms ≥ 0; otherwise empty. WAT owns
  dual scan + non-neg timeout + i64 decimal + concat. Full multi-export
  request package / multi-header still open."
  [{:keys [name params param-types result body]}]
  (and (= 3 (count params))
       ...
       (let [nm (clojure.core/name name)
             url (nth params 0)
             body-p (nth params 1)
             timeout (nth params 2)]
         (and (or (re-find #"(?i)request[_-]?edn0" nm)
                  (re-find #"(?i)request[_-]?edn[_-]?0" nm))
              ...
              (or (form-tree-walk body #(= % 'string-concat))
                  (form-tree-walk body #(= % 'edn_quoted))
                  (form-tree-walk body #(= % 'edn-quoted))
                  (form-tree-walk body #(= % 'http_request_edn))
                  (form-tree-walk body #(= % 'headers_edn_empty))
                  (form-tree-walk body #(= % "")))
              (or (form-tree-walk body #(= % "[]"))
                  (form-tree-walk body #(and (string? %) (str/includes? % "headers")))
...
(defn- classify-edn-reject-export
  ...
  (cond
    ...
    (headers-names-add-function? function) :headers-names-add
...
(defn- http-header-name-ok-function?
  "Composition for http_header_name_ok (ADR 0187) without kotoba:typed.

  Params: [name :string] Result: :i64
  Codes: -1 empty, -2 name>128, -3 non-tchar (WAT), 0 ok.
       ...
       (let [nm (first params)
             ...
             (fn []
               (and (if4? body)
                    (let [c1 (nth body 1) t1 (nth body 2) e1 (nth body 3)]
                      (and (= -1 t1)
                           (seq? c1) (contains? #{'<= '<} (first c1)) (len-of? (nth c1 1))
                           (if4? e1)
                           (let [c2 (nth e1 1) t2 (nth e1 2) e2 (nth e1 3)]
                             (and (= -2 t2)
                                  (seq? c2) (contains? #{'> '>=} (first c2)) (len-of? (nth c2 1))
                                  (#{128 129} (nth c2 2))
                                  (or (and (number? e2) (zero? e2))
                                      ;; body may continue into loop/tchar scan;
                                      ;; length skeleton is enough for admission.
...
(defn- http-header-name-ok-with-main?
  ...
  [exports]
  (let [policy (first (filter http-header-name-ok-function? exports))
        main (first (filter #(= 'main (:name %)) exports))]
    (and policy main
         (= 2 (count exports))
         (live-main-policy-calls? main (:name policy)))))
...
(defn- secret-name-ok-function?
  "Composition for secret_name_ok (ADR 0178) without kotoba:typed.

  Params: [name :string] Result: :i64
  Codes: -1 empty, -2 name>128, -3 forbidden char (WAT), 0 ok.
  Forbidden denylist: NUL TAB LF SPACE asterisk slash question backslash.
  Export name must contain the word secret. WAT applies denylist scan
  (not RFC 7230 tchar allowlist)."
  [{:keys [name params param-types result body] :as function}]
  (and (= 1 (count params))
       (= [:string] param-types)
       (= :i64 result)
       (seq? body)
       (re-find #"(?i)secret" (clojure.core/name name))
       ;; Reuse header skeleton recognition by temporarily checking without
       ;; secret exclusion: length gates -1/-2 + loop/zero.
       (let [nm (first params)
             if4? (fn [form] (and (seq? form) (= 'if (first form)) (= 4 (count form))))
             len-of? (fn [form]
                       (and (seq? form)
         ...
         (boolean
          (or
           (and (if4? body)
                (= -1 (nth body 2))
                (seq? (nth body 1)) (contains? #{'<= '<} (first (nth body 1)))
                (len-of? (nth (nth body 1) 1)))
           (and (form-tree-walk body #(= % nm))
                (form-tree-walk body #(= % -1))
                (form-tree-walk body #(= % -2))
...
(defn- http-header-value-ok-function?
  "Composition for http_header_value_ok (ADR 0188) without kotoba:typed.

  Params: [value :string] Result: :i64
  Codes: -2 value>8192, -3 CTL NUL/CR/LF (WAT), 0 ok (empty allowed).

  Frontend may desugar CTL scan to a private loop; export body keeps length
  gate + loop call. WAT always scans for CTL."
  [{:keys [params param-types result body]}]
  (and (= 1 (count params))
       (= [:string] param-types)
       (= :i64 result)
       ...
       (let [val (first params)
         ...
         (boolean
          (or
           ;; pure: (if (> (len value) 8192) -2 0)
           (and (if4? body)
                (= -2 (nth body 2))
                (seq? (nth body 1))
                (contains? #{'> '>=} (first (nth body 1)))
                (len-of? (nth (nth body 1) 1))
                (#{8192 8193} (nth (nth body 1) 2))
                (number? (nth body 3))
                (zero? (nth body 3)))
           ;; let+if length skeleton / loop call
           (and (form-tree-walk body #(= % val))
                (form-tree-walk body #(= % -2))
                (form-tree-walk body #(and (number? %) (zero? %)))
                (or (form-tree-walk body len-of?)
                    (form-tree-walk body #(= % 8192)))
                (or (form-tree-walk body #(= % -3))
                    (form-tree-walk body #(and (seq? %)
                                               (symbol? (first %))
                                               (let [s (name (first %))]
                                                 (.startsWith s "__kotoba_loop")))))))))))

(defn- http-header-pair-ok-function?
  "Composition for http_header_pair_ok (ADR 0188) without kotoba:typed.

  Params: [name :string, value :string] Result: :i64
  Codes: name -1/-2/-3; value -2→-5, -3→-6; 0 ok.

  Frontend composes private name helper + value_ok; WAT owns full policy."
  [{:keys [params param-types result body]}]
  (and (= 2 (count params))
       (= [:string :string] param-types)
       (= :i64 result)
...
(defn- http-header-value-package-with-main?
  "Three-export: value_ok + pair_ok + live main (provider shape → -3036)."
  [exports]
  (let [value (first (filter http-header-value-ok-function? exports))
...
(defn- http-request-begin-function?
  "http_request_begin []→i64: constant 0 (ADR 0193 phase need-url)."
  [{:keys [params param-types result body]}]
  (and (empty? params)
       (empty? param-types)
       (= :i64 result)
       (or (and (number? body) (zero? body))
           (and (seq? body) (= 'do (first body))
                (number? (last body)) (zero? (last body))))))

(defn- http-request-url-function?
  "http_request_url [state:i64 url:string]→i64.
  sticky; phase≠0 → -10; empty -1; >4096 -2; not https -3; else 1.
  WAT owns https:// prefix scan; recognition is skeleton."
  [{:keys [params param-types result body]}]
  (and (= 2 (count params))
...
(defn- http-request-headers-function?
  "http_request_headers [state:i64 n:i64]→i64.
  sticky; phase≠1 → -10; n∉[0,32] → -4; else 2."
  [{:keys [params param-types result body]}]
  (and (= 2 (count params))
       (= [:i64 :i64] param-types)
       (= :i64 result)
...
(defn- http-request-body-function?
  "http_request_body [state:i64 body:string]→i64.
  sticky; phase≠2 → -10; len∉[0,65536] → -5; else 3."
  [{:keys [params param-types result body]}]
  (and (= 2 (count params))
       (= [:i64 :string] param-types)
       (= :i64 result)
...
(defn- http-request-end-function?
  "http_request_end [state:i64 timeout-ms:i64]→i64.
  sticky; phase≠3 → -7; timeout∉[1,30000] → -6; else 0."
  [{:keys [params param-types result body]}]
  (and (= 2 (count params))
       (= [:i64 :i64] param-types)
       (= :i64 result)
...
(defn- http-request-pack-package-with-main?
  "Six-export: begin+url+headers+body+end + nested live main (provider → -13467)."
  [exports]
  (let [begin (first (filter http-request-begin-function? exports))
        url (first (filter http-request-url-function? exports))
        headers (first (filter http-request-headers-function? exports))
        body (first (filter http-request-body-function? exports))
        end (first (filter http-request-end-function? exports))
        main (first (filter #(= 'main (:name %)) exports))]
    (and begin url headers body end main
         (= 6 (count exports))
         (live-main-nested-policies?
          main {(:name begin) 0
                (:name url) 2
...
(defn assert-supported! [kir]
  (let [exports (exported-functions kir)
    ...
    (cond
      ...
      (and (= 1 (count exports))
           (http-header-edn-function? (first exports))
           (empty? (:effects kir))) :http-header-edn
      ;; Multi-header append + name uniqueness (composition on header_edn).
      (and (= 1 (count exports))
      ...
      (and (= 1 (count exports))
           (http-result-ok-edn-function? (first exports))
           (empty? (:effects kir))) :http-result-ok-edn
      ;; 0-header request EDN (string×string×i64 → string).
      (and (= 1 (count exports))
      ...
      (and (= 2 (count exports))
           (secret-name-ok-with-main? exports)
           (empty? (:effects kir))) :secret-name-ok-with-main
      (and (= 1 (count exports))
      ...
      (and (= 1 (count exports))
           (http-header-name-ok-function? (first exports))
           (empty? (:effects kir))) :http-header-name-ok
      (and (= 3 (count exports))
           (http-header-value-package-with-main? exports)
           (empty? (:effects kir))) :http-header-value-package-with-main
      (and (= 1 (count exports))
           (http-header-value-ok-function? (first exports))
           (empty? (:effects kir))) :http-header-value-ok
      (and (= 1 (count exports))
...
(defn- http-header-name-ok-wat
  "Composition: http_header_name_ok (ADR 0187) without kotoba:typed.

  RFC 7230 tchar charset + length [1,128]. Codes -1/-2/-3/0.
  When main-fn provided, emit live-vector main with embedded string lits."
  ([function] (http-header-name-ok-wat function nil))
  ([function main-fn]
   (let [export (wit-name (:name function))
         pages wasm/component-memory-pages
         capacity wasm/component-arena-capacity
     ...
     (str
      ...
      "    (block $name-ok\n"
      "      (loop $scan\n"
      "        local.get $i local.get $len i32.ge_u br_if $name-ok\n"
...
(defn- secret-name-ok-wat
  "Composition: secret_name_ok (ADR 0178) without kotoba:typed.

  Length [1,128] + denylist charset: NUL TAB LF SPACE * / ? \\\\
  Codes -1/-2/-3/0. When main-fn provided, emit live-vector main."
  ([function] (secret-name-ok-wat function nil))
  ([function main-fn]
   (let [export (wit-name (:name function))
         pages wasm/component-memory-pages
         capacity wasm/component-arena-capacity
     ...
     (str
      ...
      "      local.get $ptr local.get $old-ptr local.get $copy-size memory.copy\n"
      "    end local.get $ptr)\n"
      ;; denylist: 0 NUL, 9 TAB, 10 LF, 32 SPACE, 42 *, 47 /, 63 ?, 92 backslash
      "  (func $forbidden (param $c i32) (result i32)\n"
      "    local.get $c i32.eqz if i32.const 1 return end\n"
      "    local.get $c i32.const 9 i32.eq if i32.const 1 return end\n"
      "    local.get $c i32.const 10 i32.eq if i32.const 1 return end\n"
      "    local.get $c i32.const 32 i32.eq if i32.const 1 return end\n"
      "    local.get $c i32.const 42 i32.eq if i32.const 1 return end\n"
      "    local.get $c i32.const 47 i32.eq if i32.const 1 return end\n"
      "    local.get $c i32.const 63 i32.eq if i32.const 1 return end\n"
      ...
      "        local.get $i local.get $len i32.ge_u br_if $name-ok\n"
      "        local.get $ptr local.get $i i32.add i32.load8_u local.set $c\n"
      "        local.get $c call $forbidden if i64.const -3 return end\n"
      "        local.get $i i32.const 1 i32.add local.set $i\n"
      "        br $scan))\n"
      "    i64.const 0)\n"
      "  (func (export \"cm32p2||" export "\")"
...
(defn- http-header-value-package-wat
  ...
  value: empty ok; >8192 → -2; CTL 0/10/13 → -3; else 0.
  pair: name tchar/len then value with -2→-5, -3→-6."
  [value-fn pair-fn main-fn]
  (let [value-export (wit-name (:name value-fn))
    ...
    (str
     ...
     "        br $scan))\n"
     "    i64.const 0)\n"
     "  (func $pair (param $n-ptr i32) (param $n-len i32)\n"
     ...
     "    local.get $vr i64.const 0 i64.eq if i64.const 0 return end\n"
     "    local.get $vr i64.const -2 i64.eq if i64.const -5 return end\n"
     "    local.get $vr i64.const -3 i64.eq if i64.const -6 return end\n"
     "    local.get $vr)\n"
     "  (func (export \"cm32p2||" value-export "\")"
     " (param $ptr i32) (param $len i32) (result i64)\n"
     "    local.get $ptr local.get $len call $value)\n"
     "  (func (export \"cm32p2||" value-export "_post\") (param i64)\n"
     "    i32.const " arena-base " global.set $next)\n"
     "  (func (export \"cm32p2||" pair-export "\")"
     " (param $n-ptr i32) (param $n-len i32)"
     " (param $v-ptr i32) (param $v-len i32) (result i64)\n"
     "    local.get $n-ptr local.get $n-len local.get $v-ptr local.get $v-len call $pair)\n"
...
(defn- http-header-value-ok-wat
  "Composition: http_header_value_ok only (ADR 0188)."
  [function]
  (let [export (wit-name (:name function))
        pages wasm/component-memory-pages
        capacity wasm/component-arena-capacity
    ...
    (str
     ...
     "        br $scan))\n"
     "    i64.const 0)\n"
     "  (func (export \"cm32p2||" export "\") (param $ptr i32) (param $len i32) (result i64)\n"
     "    local.get $ptr local.get $len call $value)\n"
     "  (func (export \"cm32p2||" export "_post\") (param i64)\n"
     "    i32.const " arena-base " global.set $next)\n"
     "  (func (export \"cm32p2_initialize\") i32.const " arena-base " global.set $next)\n"
...
(defn- http-request-pack-package-wat
  "Six-export request packing walk + nested live main → -13467 (ADR 0193).

  States: 0 need-url, 1 need-headers, 2 need-body, 3 need-end.
  url: sticky; phase; empty -1; >4096 -2; not https:// -3; else 1.
  headers: sticky; phase; n∉[0,32] -4; else 2.
  body: sticky; phase; len∉[0,65536] -5; else 3.
  end: sticky; phase -7; timeout∉[1,30000] -6; else 0.
  begin: constant 0."
  [begin-fn url-fn headers-fn body-fn end-fn main-fn]
  (let [begin-export (wit-name (:name begin-fn))
        url-export (wit-name (:name url-fn))
        ...
        (let [vals (mapv second (partition 2 (nth (:body main-fn) 1)))]
          (vec (distinct (mapcat collect-strings vals))))
        ;; embed https:// at offset 8, then main string lits after
        prefix "https://"
        prefix-bytes (.getBytes ^String prefix StandardCharsets/UTF_8)
        prefix-len (alength prefix-bytes)
        prepared-strs
        (loop [remaining main-string-lits
               offset (align-up (+ 8 prefix-len) 1)
               acc []]
          (if-let [s (first remaining)]
            (let [b (.getBytes ^String s StandardCharsets/UTF_8)
                  len (alength b)]
        ...
        (fn emit [form]
          (cond
            (integer? form)
            (str "    i64.const " form "\n")
            (and (seq? form) (= (first form) begin-name) (= 1 (count form)))
            "    call $begin\n"
            (and (seq? form) (= (first form) url-name) (= 3 (count form)))
            (let [st (nth form 1) s (nth form 2)]
              (when-not (string? s)
                (reject "request-pack url requires string lit" {:url s}))
              (str (emit st)
            ...
            (let [st (nth form 1) s (nth form 2)]
              (when-not (string? s)
                (reject "request-pack body requires string lit" {:body s}))
              (str (emit st)
                   "    i32.const " (get str-ptr s)
                   " i32.const " (get str-len s) "\n"
                   "    call $body\n"))
            (and (seq? form) (= (first form) end-name) (= 3 (count form)))
            (str (emit (nth form 1))
                 (emit (nth form 2))
                 "    call $end\n")
            :else (reject "unsupported nested request-pack main expr" {:form form})))
        main-calls
        (let [pairs (partition 2 (nth (:body main-fn) 1))]
          (apply str
                 (map (fn [[sym call]]
    ...
    (str
     ...
     "      local.get $ptr local.get $old-ptr local.get $copy-size memory.copy\n"
     "    end local.get $ptr)\n"
     "  (func $begin (result i64) i64.const 0)\n"
     "  (func $url (param $state i64) (param $ptr i32) (param $len i32) (result i64)\n"
     "    (local $end i32) (local $i i32)\n"
     "    local.get $state i64.const 0 i64.lt_s if local.get $state return end\n"
     "    local.get $state i64.const 0 i64.ne if i64.const -10 return end\n"
     "    local.get $len i32.const " max-bytes " i32.gt_u if unreachable end\n"
     "    local.get $len i32.eqz if i64.const -1 return end\n"
     "    local.get $len i32.const 4096 i32.gt_u if i64.const -2 return end\n"
     "    local.get $len i32.const 8 i32.lt_u if i64.const -3 return end\n"
     ...
     "    local.get $len i32.const " max-bytes " i32.gt_u if unreachable end\n"
     "    local.get $len i32.const 65536 i32.gt_u if i64.const -5 return end\n"
     "    local.get $len i32.eqz if else\n"
     "      local.get $ptr i32.const 8 i32.lt_u if unreachable end\n"
     "    end\n"
     "    local.get $ptr local.get $len i32.add local.tee $end\n"
     ...
     "    local.get $end i32.const " capacity " i32.gt_u if unreachable end\n"
     "    i64.const 3)\n"
     "  (func $end (param $state i64) (param $timeout i64) (result i64)\n"
     "    local.get $state i64.const 0 i64.lt_s if local.get $state return end\n"
     "    local.get $state i64.const 3 i64.ne if i64.const -7 return end\n"
     "    local.get $timeout i64.const 1 i64.lt_s if i64.const -6 return end\n"
     "    local.get $timeout i64.const 30000 i64.gt_s if i64.const -6 return end\n"
     "    i64.const 0)\n"
     "  (func (export \"cm32p2||" begin-export "\") (result i64)\n"
     "    call $begin)\n"
     "  (func (export \"cm32p2||" begin-export "_post\") (param i64)\n"
     "    i32.const " arena-base " global.set $next)\n"
     "  (func (export \"cm32p2||" url-export "\") (param $state i64)\n"
     "    (param $ptr i32) (param $len i32) (result i64)\n"
     "    local.get $state local.get $ptr local.get $len call $url)\n"
     "  (func (export \"cm32p2||" url-export "_post\") (param i64)\n"
     "    i32.const " arena-base " global.set $next)\n"
     "  (func (export \"cm32p2||" headers-export "\") (param $state i64)\n"
     ...
     "    i32.const " arena-base " global.set $next)\n"
     "  (func (export \"cm32p2||" end-export "\") (param $state i64)\n"
     "    (param $timeout i64) (result i64)\n"
     "    local.get $state local.get $timeout call $end)\n"
     "  (func (export \"cm32p2||" end-export "_post\") (param i64)\n"
     "    i32.const " arena-base " global.set $next)\n"
     "  (func (export \"cm32p2||main\") (result i64)\n"
     "    " main-locals "\n"
...
(defn- http-header-edn-wat
  "Canonical `string×string -> string` reject-path EDN header map.

  Dual UTF-8 scan for quote/backslash on name and value; if either fails,
  returns empty. Else builds `{:name \"…\" :value \"…\"}` via realloc copies
  (composition on edn_quoted semantics; no kotoba:typed)."
  [function]
  (let [export (wit-name (:name function))
        pages wasm/component-memory-pages
        capacity wasm/component-arena-capacity
        max-bytes value/string-value-byte-limit
        ;; literals packed from offset 8
        ;; pref = "{:name \""  (8 bytes)
        ;; mid  = "\" :value \"" (10 bytes)
        ;; suf  = "\"}" (2 bytes)
        pref-ptr 8
        ...
        suf-len 2
        arena-base 32
        fixed-overhead (+ pref-len mid-len suf-len)]
    (str
     "(module\n"
     "  (memory (export \"cm32p2_memory\") " pages " " pages ")\n"
     "  (global $next (mut i32) (i32.const " arena-base "))\n"
     ...
     "    (local $end i32) (local $out i32) (local $ret i32) (local $total i32) (local $cursor i32)\n"
     ...
     "    end\n"
     "    local.get $v-len i32.eqz if else\n"
     "      local.get $v-ptr i32.const 8 i32.lt_u if unreachable end\n"
     "    end\n"
     ...
     "    if call $empty-string return end\n"
     "    local.get $n-len local.get $v-len i32.add i32.const " fixed-overhead " i32.add local.set $total\n"
     "    local.get $total i32.const " max-bytes " i32.gt_u if unreachable end\n"
     "    i32.const 0 i32.const 0 i32.const 1 local.get $total call $realloc local.set $out\n"
     "    i32.const 0 local.set $cursor\n"
     ...
     "    local.get $out local.get $cursor i32.add local.get $n-ptr local.get $n-len memory.copy\n"
     "    local.get $cursor local.get $n-len i32.add local.set $cursor\n"
     "    local.get $out local.get $cursor i32.add i32.const " mid-ptr " i32.const " mid-len " memory.copy\n"
     "    local.get $cursor i32.const " mid-len " i32.add local.set $cursor\n"
     "    local.get $out local.get $cursor i32.add local.get $v-ptr local.get $v-len memory.copy\n"
...
(defn- headers-edn-append-wat
  ...
  (let [export (wit-name (:name function))
    ...
    (str
     ...
     "    local.get $ret i32.const 0 i32.store offset=4\n"
     "    local.get $ret)\n"
     "  ;; has-prefix-at? haystack[i..] starts with needle\n"
     ...
     "    i32.const 0 local.set $cursor\n"
     "    local.get $h local.get $cursor i32.add i32.const " pref-ptr " i32.const " pref-len " memory.copy\n"
     "    local.get $cursor i32.const " pref-len " i32.add local.set $cursor\n"
     "    local.get $h local.get $cursor i32.add local.get $n-ptr local.get $n-len memory.copy\n"
     "    local.get $cursor local.get $n-len i32.add local.set $cursor\n"
     "    local.get $h local.get $cursor i32.add i32.const " mid-ptr " i32.const " mid-len " memory.copy\n"
     "    local.get $cursor i32.const " mid-len " i32.add local.set $cursor\n"
     "    local.get $h local.get $cursor i32.add local.get $v-ptr local.get $v-len memory.copy\n"
     "    local.get $cursor local.get $v-len i32.add local.set $cursor\n"
     "    local.get $h local.get $cursor i32.add i32.const " suf-ptr " i32.const " suf-len " memory.copy\n"
     "    ;; empty vec vs append\n"
     ...
     "      local.get $out local.get $h-len i32.add i32.const 1 i32.add\n"
...
(defn- http-result-ok-edn-wat
  "Canonical `i64×string×string -> string` reject-path result ok arm.

  Params: status (i64 0..999), headers (`[…]` shape), body (no quote/backslash).
  Builds `{:tag :ok :status N :headers … :body \"…\"}`. Decimal status for
  0–999 only. No kotoba:typed."
  [function]
  (let [export (wit-name (:name function))
        pages wasm/component-memory-pages
        capacity wasm/component-arena-capacity
        max-bytes value/string-value-byte-limit
        ;; literals from offset 8
        ;; pref = "{:tag :ok :status " (18 incl trailing space)
        ;; mid1 = " :headers " (10)
        ;; mid2 = " :body \"" (8)
        ;; suf  = "\"}" (2)
        ...
        digits-len 10
        arena-base 64
        fixed-overhead (+ pref-len mid1-len mid2-len suf-len)]
    (str
     "(module\n"
     "  (memory (export \"cm32p2_memory\") " pages " " pages ")\n"
     "  (global $next (mut i32) (i32.const " arena-base "))\n"
     "  (data (i32.const " pref-ptr ") \"{:tag :ok :status \")\n"
     ...
     "    i32.const 1)\n"
     "  ;; write decimal 0..999 into $buf; return digit count in result\n"
     "  (func $write-status (param $status i32) (param $buf i32) (result i32)\n"
     "    (local $n i32) (local $d0 i32) (local $d1 i32) (local $d2 i32)\n"
     "    local.get $status i32.const 10 i32.lt_u\n"
     "    if\n"
     "      local.get $buf i32.const " digits-ptr " local.get $status i32.add i32.const 1 memory.copy\n"
     "      i32.const 1 return\n"
     "    end\n"
     "    local.get $status i32.const 100 i32.lt_u\n"
     "    if\n"
     "      local.get $status i32.const 10 i32.div_u local.set $d1\n"
     "      local.get $status local.get $d1 i32.const 10 i32.mul i32.sub local.set $d0\n"
     "      local.get $buf i32.const " digits-ptr " local.get $d1 i32.add i32.const 1 memory.copy\n"
     "      local.get $buf i32.const 1 i32.add i32.const " digits-ptr " local.get $d0 i32.add i32.const 1 memory.copy\n"
     "      i32.const 2 return\n"
     "    end\n"
     "    local.get $status i32.const 100 i32.div_u local.set $d2\n"
     "    local.get $status local.get $d2 i32.const 100 i32.mul i32.sub local.set $n\n"
     "    local.get $n i32.const 10 i32.div_u local.set $d1\n"
     "    local.get $n local.get $d1 i32.const 10 i32.mul i32.sub local.set $d0\n"
     "    local.get $buf i32.const " digits-ptr " local.get $d2 i32.add i32.const 1 memory.copy\n"
     "    local.get $buf i32.const 1 i32.add i32.const " digits-ptr " local.get $d1 i32.add i32.const 1 memory.copy\n"
     "    local.get $buf i32.const 2 i32.add i32.const " digits-ptr " local.get $d0 i32.add i32.const 1 memory.copy\n"
     "    i32.const 3)\n"
     "  (func (export \"cm32p2||" export "\")"
     " (param $status i64)"
     " (param $h-ptr i32) (param $h-len i32)"
     " (param $b-ptr i32) (param $b-len i32) (result i32)\n"
     "    (local $end i32) (local $out i32) (local $ret i32) (local $total i32)\n"
     "    (local $cursor i32) (local $s-len i32) (local $s-buf i32) (local $st i32)\n"
     "    local.get $h-len i32.const " max-bytes " i32.gt_u if unreachable end\n"
     ...
     "    local.get $end i32.const " capacity " i32.gt_u if unreachable end\n"
     "    ;; status gate [0,999]\n"
     "    local.get $status i64.const 0 i64.lt_s if call $empty-string return end\n"
     "    local.get $status i64.const 999 i64.gt_s if call $empty-string return end\n"
     "    local.get $status i32.wrap_i64 local.set $st\n"
     ...
     "    if call $empty-string return end\n"
     "    ;; status digits into temp arena buf (3 max)\n"
     "    i32.const 0 i32.const 0 i32.const 1 i32.const 4 call $realloc local.set $s-buf\n"
     "    local.get $st local.get $s-buf call $write-status local.set $s-len\n"
     "    local.get $s-len local.get $h-len i32.add local.get $b-len i32.add\n"
     "    i32.const " fixed-overhead " i32.add local.set $total\n"
     "    local.get $total i32.const " max-bytes " i32.gt_u if unreachable end\n"
     "    i32.const 0 i32.const 0 i32.const 1 local.get $total call $realloc local.set $out\n"
     "    i32.const 0 local.set $cursor\n"
     "    local.get $out local.get $cursor i32.add i32.const " pref-ptr " i32.const " pref-len " memory.copy\n"
     "    local.get $cursor i32.const " pref-len " i32.add local.set $cursor\n"
     "    local.get $out local.get $cursor i32.add local.get $s-buf local.get $s-len memory.copy\n"
     "    local.get $cursor local.get $s-len i32.add local.set $cursor\n"
...
(defn- http-request-edn0-wat
  "Canonical `string×string×i64 -> string` reject-path 0-header request EDN.

  Params: url, body, timeout-ms. Dual quote/backslash scan; reject timeout < 0.
  Else builds `{:url \"…\" :headers [] :body \"…\" :timeout-ms N}` with decimal
  timeout. Composition on edn_quoted; multi-header / full multi-export open."
  [function]
  (let [export (wit-name (:name function))
        pages wasm/component-memory-pages
        capacity wasm/component-arena-capacity
    ...
    (str
     ...
     "  (data (i32.const " mid1-ptr ") \"\\\" :headers [] :body \\\"\")\n"
     "  (data (i32.const " mid2-ptr ") \"\\\" :timeout-ms \")\n"
     "  (data (i32.const " suf-ptr ") \"}\")\n"
     "  (data (i32.const " digits-ptr ") \"0123456789\")\n"
     "  (func $realloc (export \"cm32p2_realloc\")\n"
     "    (param $old-ptr i32) (param $old-size i32)\n"
     ...
     "    local.get $ret i32.const 0 i32.store offset=4\n"
     "    local.get $ret)\n"
     "  ;; Write non-negative i64 decimal into $out; return digit count.\n"
     ...
     "    local.get $b-ptr local.get $b-len call $has-forbidden\n"
     "    if call $empty-string return end\n"
     "    ;; digit count: 1 for 0 else floor(log10)+1\n"
     ...
     "        end)\n"
     "    end\n"
     "    local.get $u-len local.get $b-len i32.add\n"
...
(defn- http-edn-reject-package-wat
  ...
  (let [pages wasm/component-memory-pages
        ...
        (str
         ...
         "  (data (i32.const " digits-ptr ") \"0123456789\")\n"
         "  (data (i32.const " pref-ok-ptr ") \"{:tag :ok :status \")\n"
         "  (data (i32.const " mid-ok1-ptr ") \" :headers \")\n"
         "  (data (i32.const " mid-ok2-ptr ") \" :body \\\"\")\n"
         "  (data (i32.const " suf-ok-ptr ") \"\\\"}\")\n"
         "  (data (i32.const " pref-err-ptr ") \"{:tag :error :code \\\"\")\n"
         "  (data (i32.const " mid-false-ptr ") \"\\\" :retryable false}\")\n"
         "  (data (i32.const " mid-true-ptr ") \"\\\" :retryable true}\")\n"
         "  (func $realloc (export \"cm32p2_realloc\")\n"
         "    (param $old-ptr i32) (param $old-size i32)\n"
         ...
         "        br $scan))\n"
         "    i32.const 0)\n"
         "  ;; Element-bound name membership for true-set name lists (ADR 0225)\n"
         "  (func $has-name-element (param $a-ptr i32) (param $a-len i32)\n"
         "    (param $n-ptr i32) (param $n-len i32) (result i32)\n"
         "    (local $i i32) (local $prev i32) (local $after i32) (local $c i32)\n"
         "    local.get $n-len i32.const 2 i32.add local.get $a-len i32.gt_u if i32.const 0 return end\n"
          ...
          (let [export (wit-name (:name f))]
            (case role
              ...
              (str
               ...
               "    local.get $h-ptr local.get $h-len call $headers-shape-ok i32.eqz if call $empty-string return end\n"
               "    local.get $b-ptr local.get $b-len call $has-forbidden if call $empty-string return end\n"
               "    local.get $status i64.const 0 i64.eq if i32.const 1 local.set $dlen else\n"
               "      local.get $status local.set $tmp i32.const 0 local.set $dlen\n"
               "      (loop $c local.get $tmp i64.const 0 i64.eq if else\n"
               "        local.get $dlen i32.const 1 i32.add local.set $dlen\n"
               "        local.get $tmp i64.const 10 i64.div_u local.set $tmp br $c end)\n"
               "    end\n"
               "    local.get $dlen local.get $h-len i32.add local.get $b-len i32.add\n"
               "    i32.const " ok-overhead " i32.add local.set $total\n"
               "    local.get $total i32.const " max-bytes " i32.gt_u if unreachable end\n"
               "    i32.const 0 i32.const 0 i32.const 1 local.get $total call $realloc local.set $out\n"
               "    i32.const 0 local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add i32.const " pref-ok-ptr " i32.const " pref-ok-len " memory.copy\n"
               "    local.get $cursor i32.const " pref-ok-len " i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add local.get $status call $write-u64 drop\n"
               "    local.get $cursor local.get $dlen i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add i32.const " mid-ok1-ptr " i32.const " mid-ok1-len " memory.copy\n"
               "    local.get $cursor i32.const " mid-ok1-len " i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add local.get $h-ptr local.get $h-len memory.copy\n"
               "    local.get $cursor local.get $h-len i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add i32.const " mid-ok2-ptr " i32.const " mid-ok2-len " memory.copy\n"
               "    local.get $cursor i32.const " mid-ok2-len " i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add local.get $b-ptr local.get $b-len memory.copy\n"
               "    local.get $cursor local.get $b-len i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add i32.const " suf-ok-ptr " i32.const " suf-ok-len " memory.copy\n"
               "    i32.const 0 i32.const 0 i32.const 4 i32.const 8 call $realloc local.tee $ret\n"
               "    local.get $out i32.store\n"
               "    local.get $ret local.get $total i32.store offset=4\n"
               "    local.get $ret)\n"
              ...
              (str
               ...
               "    (local $cursor i32) (local $mid-ptr i32) (local $mid-len i32)\n"
               "    local.get $c-len i32.const " max-bytes " i32.gt_u if unreachable end\n"
               "    local.get $c-len i32.eqz if else local.get $c-ptr i32.const 8 i32.lt_u if unreachable end end\n"
               "    local.get $c-ptr local.get $c-len i32.add local.tee $end local.get $c-ptr i32.lt_u if unreachable end\n"
               "    local.get $end i32.const " capacity " i32.gt_u if unreachable end\n"
               "    local.get $retry i64.const 0 i64.eq if else\n"
               "      local.get $retry i64.const 1 i64.eq if else call $empty-string return end end\n"
               "    local.get $c-ptr local.get $c-len call $has-forbidden if call $empty-string return end\n"
               "    local.get $retry i64.const 0 i64.eq\n"
               "    if\n"
               "      i32.const " mid-false-ptr " local.set $mid-ptr\n"
               "      i32.const " mid-false-len " local.set $mid-len\n"
               ...
               "      i32.const " mid-true-len " local.set $mid-len\n"
               "    end\n"
               "    local.get $c-len i32.const " pref-err-len " i32.add local.get $mid-len i32.add local.set $total\n"
               "    local.get $total i32.const " max-bytes " i32.gt_u if unreachable end\n"
               "    i32.const 0 i32.const 0 i32.const 1 local.get $total call $realloc local.set $out\n"
               "    i32.const 0 local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add i32.const " pref-err-ptr " i32.const " pref-err-len " memory.copy\n"
               "    local.get $cursor i32.const " pref-err-len " i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add local.get $c-ptr local.get $c-len memory.copy\n"
               "    local.get $cursor local.get $c-len i32.add local.set $cursor\n"
               "    local.get $out local.get $cursor i32.add local.get $mid-ptr local.get $mid-len memory.copy\n"
               "    i32.const 0 i32.const 0 i32.const 4 i32.const 8 call $realloc local.tee $ret\n"
              ...
              (str
               ...
               "    local.get $a-len i32.eqz if else local.get $a-ptr i32.const 8 i32.lt_u if unreachable end end\n"
               "    local.get $n-len i32.eqz if else local.get $n-ptr i32.const 8 i32.lt_u if unreachable end end\n"
               "    local.get $a-ptr local.get $a-len i32.add local.tee $end local.get $a-ptr i32.lt_u if unreachable end\n"
               "    local.get $end i32.const " capacity " i32.gt_u if unreachable end\n"
               "    local.get $n-ptr local.get $n-len i32.add local.tee $end local.get $n-ptr i32.lt_u if unreachable end\n"
               "    local.get $end i32.const " capacity " i32.gt_u if unreachable end\n"
               "    local.get $a-len i32.eqz if call $empty-string return end\n"
               "    local.get $n-len i32.eqz if call $empty-string return end\n"
               "    local.get $n-ptr local.get $n-len call $has-forbidden if call $empty-string return end\n"
               "    local.get $a-ptr local.get $a-len local.get $n-ptr local.get $n-len call $has-name-element\n"
               "    if call $empty-string return end\n"
               "    local.get $n-len i32.const 2 i32.add local.set $elem-len\n"
               "    local.get $elem-len i32.const " max-bytes " i32.gt_u if unreachable end\n"
               "    local.get $a-ptr local.get $a-len call $is-empty-vec\n"
               ...
               "  (func (export \"cm32p2||" export "_post\") (param i32)\n"
               "    i32.const " arena-base " global.set $next)\n")
              ;; optional roles not in this package slice
...
(def ^:private synthetic-wall-base
  "Fixed Unix-ms origin for the synthetic clock. Its presence in emitted WAT
  is how a test tells the two time sources apart."
  1700000000000)

(defn- clock-provider-module
  "One provider core module for clock-v1's own literal request/result shape,
  parameterized by where the time comes from.

  There used to be two near-identical copies of this function, and the repo
  rule they broke is the one about mirrors: the same decision held in two
  implementations is a decision that gets fixed in one of them. Everything
  here -- shape admission, layouts, the bounded allocator, the observation
  counter, the arena reset, the reserved error literals -- is common, and
  keeping it common is what makes the two providers substitutable rather
  than merely similar.

  A `source` supplies only the difference:

  | key | meaning |
  |---|---|
  | `:imports` | WAT import lines, or `\"\"` for a self-contained module |
  | `:globals` | extra mutable globals the source needs |
  | `:scratch-bytes` | return area reserved below the arena, for imports that return indirectly |
  | `:wall-branch` | emits the whole `wall` arm, given the offsets |
  | `:mono-branch` | emits the whole `monotonic` arm |

  The branches are handed the arm rather than an expression because the two
  sources genuinely order their stores differently, and forcing a shared
  order here would rewrite emitted bytes for the sake of symmetry that is
  not there."
  [entry request-descriptor result-descriptor schemas source]
  ...
  (let [export (str "cm32p2|kotoba:application/" (:interface entry) "@1|" (:function entry))
        ...
        arena-base (align-up (+ scratch-pointer (:scratch-bytes source)) 8)
        result-size (:size result-layout)
        ...
        offsets {:scratch scratch-pointer
                 :disc-store disc-store
                 :wall-millis (+ payload-offset (:offset (field-by-name wall-layout :unix-millis)))
                 :wall-obs (+ payload-offset (:offset (field-by-name wall-layout :observation-sequence)))
                 :mono-nanos (+ payload-offset (:offset (field-by-name mono-layout :nanos)))
                 :mono-obs (+ payload-offset (:offset (field-by-name mono-layout :observation-sequence)))}]
    (str
     "(module\n"
     (:imports source)
     "  (memory (export \"cm32p2_memory\") " pages " " pages ")\n"
     ...
     "    local.get $disc i32.const 0 i32.eq\n"
     "    if\n"
     ((:wall-branch source) offsets)
     "    else\n"
     ((:mono-branch source) offsets)
     "    end\n"
     "    local.get $ret)\n"
     "  (func (export \"" export "_post\") (param i32)\n"
     "    i32.const " arena-base " global.set $next)\n"
...
(def ^:private synthetic-clock-source
  "Self-contained, deterministic sources. Documented policy:
  ...
  {:imports ""
   :globals (str "  (global $wall (mut i64) (i64.const " synthetic-wall-base "))\n"
                 "  (global $mono (mut i64) (i64.const 0))\n")
   :scratch-bytes 0
   :wall-branch
   (fn [{:keys [disc-store wall-millis wall-obs]}]
     (str "      global.get $wall i64.const 1 i64.add global.set $wall\n"
          "      local.get $ret i32.const 0 " disc-store " offset=0\n"
          "      local.get $ret global.get $wall i64.store offset=" wall-millis "\n"
          "      local.get $ret global.get $obs i64.store offset=" wall-obs "\n"))
   :mono-branch
   (fn [{:keys [disc-store mono-nanos mono-obs]}]
     (str "      global.get $mono i64.const 1000 i64.add global.set $mono\n"
          "      local.get $ret i32.const 1 " disc-store " offset=0\n"
          "      local.get $ret global.get $mono i64.store offset=" mono-nanos "\n"
          "      local.get $ret global.get $obs i64.store offset=" mono-obs "\n"))})

(def ^:private wasi-clock-source
  "Real host time through WASI 0.3.

  `system-clock.now` returns `instant {seconds:s64, nanoseconds:u32}`. Two
  flat results exceed MAX_FLAT_RESULTS, so the Canonical ABI returns it
  indirectly: the core import takes the address of a caller-owned return
  area, and `:scratch-bytes` reserves 16 bytes below the arena for it so it
  can never collide with a `$realloc` result. `monotonic-clock.now` returns
  `mark (u64)`, one flat result, returned directly.
  ...
  {:imports (str "  (import \"" wasi-system-clock-import-module "\" \"now\""
   ...
   :scratch-bytes 16
   :wall-branch
   (fn [{:keys [scratch disc-store wall-millis wall-obs]}]
     (str "      i32.const " scratch " call $wasi-system-now\n"
          "      local.get $ret\n"
          "      i32.const " scratch " i64.load offset=0 i64.const 1000 i64.mul\n"
          "      i32.const " scratch " i32.load offset=8"
          " i64.extend_i32_u i64.const 1000000 i64.div_u\n"
          "      i64.add\n"
          "      i64.store offset=" wall-millis "\n"
          "      local.get $ret i32.const 0 " disc-store " offset=0\n"
          "      local.get $ret global.get $obs i64.store offset=" wall-obs "\n"))
   :mono-branch
   (fn [{:keys [disc-store mono-nanos mono-obs]}]
     (str "      local.get $ret call $wasi-monotonic-now i64.store offset=" mono-nanos "\n"
          "      local.get $ret i32.const 1 " disc-store " offset=0\n"
          "      local.get $ret global.get $obs i64.store offset=" mono-obs "\n"))})

(defn clock-provider-wat
  "REAL (non-wiring-only) provider core module for clock-v1's own literal
  request/result shape, with synthetic wall/monotonic sources and a real
  observation-sequence.

  Production host wall/monotonic injection remains the CLJ/CLJS transport
  path (ADR 0073). For host time inside the component itself, see
  `clock-wasi-provider-wat`. This is wasm-component qualification for the
  ABI + sequence semantics, not a production host-time claim."
  [entry request-descriptor result-descriptor schemas]
  (clock-provider-module entry request-descriptor result-descriptor schemas
                         synthetic-clock-source))

(defn clock-wasi-provider-wat
  "Provider core module backed by REAL host time through WASI 0.3 imports
  rather than the synthetic sources in `clock-provider-wat`.

  Two imports, and only these two: `wasi:clocks/system-clock@0.3.0` and
  `wasi:clocks/monotonic-clock@0.3.0`, the interfaces
  `component-model-v1.edn` declares for `:clock/now`. The provider gets no
  filesystem, socket, environment, or random authority, and the application
  component that composes with it gets no WASI at all -- it still sees only
  `kotoba:application/clock`. An engine that does not grant the clocks fails
  to link before instantiation, which is the intended denial path.
...
(defn emit
  ...
  (case (assert-supported! kir)
    ...
    :http-result-ok-edn
    (wasm-tools/parse-wat
     (http-result-ok-edn-wat (first (exported-functions kir))))
    :http-request-edn0
    (wasm-tools/parse-wat
    ...
    :secret-name-ok
    (wasm-tools/parse-wat
     (secret-name-ok-wat (first (exported-functions kir))))
    ...
    :http-header-name-ok-with-main
    (let [exports (exported-functions kir)
          policy (first (filter http-header-name-ok-function? exports))
          main (first (filter #(= 'main (:name %)) exports))]
      (wasm-tools/parse-wat (http-header-name-ok-wat policy main)))
    :http-header-value-ok
    (wasm-tools/parse-wat
     (http-header-value-ok-wat (first (exported-functions kir))))
    :http-header-value-package-with-main
    (let [exports (exported-functions kir)
          value (first (filter http-header-value-ok-function? exports))
          pair (first (filter http-header-pair-ok-function? exports))
          main (first (filter #(= 'main (:name %)) exports))]
    ...
    :http-request-pack-package-with-main
    (let [exports (exported-functions kir)
          begin (first (filter http-request-begin-function? exports))
          url (first (filter http-request-url-function? exports))
          headers (first (filter http-request-headers-function? exports))
          body (first (filter http-request-body-function? exports))
          end (first (filter http-request-end-function? exports))
